...
namespace XaeiO.Compiler.CodeModel
{
    public static class CodeUtility
    {
        ...
        public static T CloneCode<T>(T obj) where T : ICodeObject
        {
            Type type = obj.GetType();
            T clone = (T)Activator.CreateInstance(type, true);
            foreach (Reflection.PropertyInfo propertyInfo in obj.GetType().GetProperties())
            {
                object value = propertyInfo.GetValue(obj, null);

                if (value == null)
                {
                    SetProperty(propertyInfo, clone, null);
                    continue;
                }

                if (object.ReferenceEquals(value, obj))
                {
                    SetProperty(propertyInfo, clone, clone);
                    continue;
                }
                ...
                if (expression != null)
                {
                    SetProperty(propertyInfo, clone, CloneCode<IExpression>(expression));
                    continue;
                }
                ...
                if (statement != null)
                {
                    SetProperty(propertyInfo, clone, CloneCode<IStatement>(statement));
                    continue;
                }
                ...
                if (expressions != null)
                {
                    List<IExpression> clonedExpressions = new List<IExpression>();
                    foreach (IExpression innerExpression in expressions)
                    {
                        clonedExpressions.Add(CloneCode<IExpression>(innerExpression));
                    }
                    SetProperty(propertyInfo, clone, clonedExpressions);
                    continue;
                }
                
                IList<IStatement> statements = value as IList<IStatement>;
                if (statements != null)
                {
                    List<IStatement> clonedStatements = new List<IStatement>();
                    foreach (IStatement innerStatement in statements)
                    {
                        clonedStatements.Add(CloneCode<IStatement>(innerStatement));
                    }
                    SetProperty(propertyInfo, clone, clonedStatements);
                    continue;
                }

                IList<ISwitchCase> cases = value as IList<ISwitchCase>;
                if (cases != null)
                {
                    List<ISwitchCase> clonedCases = new List<ISwitchCase>();
                    foreach (ISwitchCase innerCase in cases)
                    {
                        clonedCases.Add(CloneCode<ISwitchCase>(innerCase));
                    }
                    SetProperty(propertyInfo, clone, clonedCases);
                    continue;
                }

                IList<ICatchClause> clauses = value as IList<ICatchClause>;
                if (clauses != null)
                {
                    List<ICatchClause> clonedClauses = new List<ICatchClause>();
                    foreach (ICatchClause innerClause in clauses)
                    {
                        clonedClauses.Add(CloneCode<ICatchClause>(innerClause));
                    }
                    SetProperty(propertyInfo, clone, clonedClauses);
                    continue;
                }

                //Reflection.MethodInfo memberwiseClone = value.GetType().GetMethod("MemberwiseClone", Reflection.BindingFlags.Instance | Reflection.BindingFlags.NonPublic);
                SetProperty(propertyInfo, clone, value);
            }

            return clone;
        }

        private static void SetProperty(Reflection.PropertyInfo propertyInfo, ICodeObject target, object value)
        {
            try
            {
            ...
            catch (Reflection.TargetInvocationException e)
            {
                ...
                {
                    // many ICodeObjects don't support setters on certain values
                    // ignore this exception
                    // TODO: Use a special code cloning attribute on the property instead
                }
                else
                {
                    throw;
